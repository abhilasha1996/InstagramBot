...
ACCESS_TOKEN = '3556367448.72e3852.9519ab577e384a4fa6c74f8e4f2e8e04'
...
def get_user_info(insta_username):
    ...
    if user_info['meta']['code'] == 200:
        ...
        print 'The profile of the user is private....so ask him/her to remove security from pfofile photo to see posts info:'
...
# fuction to delete a negative comment:
...
# find the caption and the trend and subtrend of the post:
def caption(insta_username):
    u_id = get_user_id(insta_username)
    post_id = get_post_id(insta_username)
    if u_id == None:
        print 'User does not exist!'
        exit()
    #https: // api.instagram.com / v1 / users / {user - id} / media / recent /?access_token = ACCESS - TOKEN
    request_url = (BASE_URL + 'users/%s/media/recent/?access_token=%s')%( u_id,ACCESS_TOKEN)
    print 'GET request url : %s' % (request_url)
    user_media = requests.get(request_url).json()
    print user_media
    ...
    if user_media['meta']['code'] == 200:
        if len(user_media['data']):
            var=user_media['data'][0]['caption']['text']
            print var
            var2=var.upper()
            olympic_games_list =['FOOTBALL','BADMINTON','VOLLYBALL','CHESS','TENNIS','BASKETBALL','ATHLETICS','SWIMMING','BOXING','CYCLING',]
            for x in olympic_games_list:
                if(var2.find(x)>=0):
                    print "Trend is:"
                    print x
                    break
...
def start_bot():
    while True:
        ...
        print "f.Get a recent post's post id\n"
        print "g.Like the recent post of a user\n"
        print "h.comment on the recent post of the user\n"
        print "i.Delete a negative comment\n"
        print "j. Read a caption of the post\n"
        print "k.Exit"
